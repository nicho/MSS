...
<script type="text/javascript">
	/**图书名、图书ID***/
	function btn_click(bookName,bookID,author,bookConcern,bookCode){
		parent.bookmasterSetValue(bookName,bookID,author,bookConcern,bookCode);
		parent.closeDialog();
	}
</script>
<style>
...
<body style="border:0px;overflow:auto;height:400px;">
 <form id='searchForm' class="form-search" action="${ctx}/getSelectBookMaster.do">
		<div class="add_top" style>&nbsp;&nbsp;&nbsp;●&nbsp;&nbsp;上架书籍选择</div>
		<!--搜索部分begin-->	
		 <div class="row-fluid aufour mar_2"> 
				...
				<input type="hidden" name="url" value="${param.url}"/>
			     <table  class="tableopen"  width="100%">
			    	...
			    	<tr>
			    	    <td class="text_rg title_font bzh">内部编码</td>
			    		<td class="text_lf">
			    			<input   type="text" class="open_srk  " name="bookCode" id="bookCode" value="${param.bookCode}">
			    	</tr>			    				    	
		    	</table>
	    	...
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	<tr class="tbth">
			    		...
			    		<td class="bzh">作者</td>
			    		<td class="bzh">出版社</td>
			    		<td class="bzh">内部编码</td>  
			    		<td class="bzh">上架时间</td>
			    		
			    	</tr>
			    	<c:forEach var="list"  items="${v_booksList}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.bookName}','${list.bookID}','${list.author}','${list.bookConcern}','${list.bookCode}')">选择</a>
						    </td>
				    		<td>${list.bookName}&nbsp;</td>
				    		<td>${list.author}&nbsp;</td>
				    		<td>${list.bookConcern}&nbsp;</td>
				    		<td>${list.bookCode}&nbsp;</td>
				    		<td>${list.putonShelf}&nbsp;</td>
				    	</tr>
				    </c:forEach> 
			    </table>    
		   		<tags:paginationOpen/>
