...
<script type="text/javascript">
	function btn_click(bcBudgetHId,budgetName) {
		parent.selectBudgetHSet(bcBudgetHId, budgetName);
		parent.closeDialog2();
	}
</script>
<style>
...
<body style="border: 0px; overflow: auto; height: 500px; height: 480px\9;">
	<form class="form-search" action="${ctx}/viewSelectBudgetH.do">
		<div class="add_top">&nbsp;&nbsp;&nbsp;●&nbsp;&nbsp;预算</div>
		<!--搜索部分begin-->
		<div class="row-fluid aufour mar_2">
			<input type="hidden" name="orgId" value="${param.orgId}" /> <input type="hidden" name="channelType" value="${param.channelType}" /> <input type="hidden" name="url" value="${param.url}" />

			<table class="tableopen" width="100%">
				<tr>
					<td class="text_rg title_font bzh ">预算名称</td>
					<td class="text_lf"><input type="text" class="open_srk" name="bc_budget_h_name" id="bc_budget_h_name" value="${param.bc_budget_h_name}" /></td>
		...
		<div class="row-fluid aufive mar_2">
			...
			<div class="table_auto scroll_auto" style="overflow-x: auto; overflow-y: hidden;">
				<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
					...
					<c:forEach var="list" items="${budgetHList}" varStatus="vstatus">
						<tr>
							<td><a onclick="btn_click('${list.bcBudgetHId}','${list.budgetName}')">选择</a></td>
							<td>${list.orgName}</td>
							<td>${list.channelName}</td>
							<td>${list.budgetName}</td>
							<td>${list.amountName}</td>
							<td>${list.boundaryName}</td>
							<td><c:if test="${list.detailDepartment == 'Y'}">是</c:if></td>
							<td>${list.startYear}</td>
							<td>${list.endYear}</td>
						</tr>
					</c:forEach>
				</table>
