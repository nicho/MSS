...
<script type="text/javascript">
	function btn_click(accountNumber,accountName,departmentCode,orgId,channelType,secInvName,description){
		parent.custAcountSet(accountNumber,accountName,departmentCode,orgId,channelType,secInvName,description);
		parent.closeDialog();
	}
</script>
<style>
	...
	<body style="border:0px;overflow:auto;height:500px;height:480px\9;">
		...
		<!--搜索部分begin-->	
		 <div class="row-fluid aufour mar_2"> 
		   	<form class="form-search" action="${ctx}/viewBcCustAccountList.do">
				...
				<input type="hidden" name="orgId" value="${param.orgId}"/>
					<input type="hidden" name="departmentCode" value="${param.departmentCode}"/>
	    	...
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	...
			    	<c:forEach var="list"  items="${custAccountList}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.accountNumber}','${list.accountName}','${list.departmentCode}','${list.orgId}','${list.channelType}','${list.secInvName}','${list.description}')">选择</a>
						    </td>
