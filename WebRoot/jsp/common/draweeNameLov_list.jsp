...
<script type="text/javascript">
	function btn_click(code,text){
		parent.draweeNameLovSet(code,text);
		parent.closeDialog();
	}
</script>
<style>
...
.open_xlk{width:200px;height:25px;overflow:hidden;border:1px solid #ccc;margin-left:8px\9;font-size:12px!important;}
	...
	<body style="border:0px;overflow:auto;height:500px;height:480px\9;">
		<div class="add_top" style>&nbsp;&nbsp;&nbsp;●&nbsp;&nbsp;费用承担方名称</div>
		<!--搜索部分begin-->	
		 <div class="row-fluid aufour mar_2"> 
		   	<form class="form-search" action="${ctx}/getDraweeNameLovList.do">
				<input type="hidden" name="orgId" id="orgId" value="${param.organization_id}"/>
				<input type="hidden" name="type" id="type" value="${param.type}"/>
				<input type="hidden" name="url" id="url" value="${param.url}"/>
			     <table  class="tableopen"  width="100%">
			    	<tr>
			    	    <td class="text_rg title_font bzh">编码</td>
			    		<td class="text_lf">
			    		<input type="text" class="open_srk " name="code" id="code" value="${param.code}" />  
				         </td>
			    	    <td class="text_rg title_font bzh ">名称</td>
			    		<td class="text_lf">
			    			<input type="text" class="open_srk " name="text" id="text" value="${param.text}" />
			    		</td>
			    	</tr>
		    	</table>
		 ...
		 <div class="row-fluid aufive mar_2"> 
		   <div class="aulist_3" ><b></b>费用承担方列表</div>  
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	...
			    	<c:forEach var="list"  items="${list}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.code}','${list.text}')">选择</a>
						    </td>
				    		<td title="${list.code}">${list.code}</td>
				    		<td title="${list.text}">${list.text}</td>     		
