...
<script type="text/javascript">
	function btn_click(roleId,roleName){
		parent.roleInfoSet(roleId,roleName);
		parent.closeDialog();
	}
</script>
<style>
...
.open_xlk{width:200px;height:25px;overflow:hidden;border:1px solid #ccc;margin-left:8px\9;font-size:12px!important;}
	...
	<body style="border:0px;overflow:auto;height:500px;height:480px\9;">
		<div class="add_top" style>&nbsp;&nbsp;&nbsp;●&nbsp;&nbsp;角色选择</div>
		<!--搜索部分begin-->	
		 <div class="row-fluid aufour mar_2"> 
		   	<form class="form-search" action="${ctx}/getAllRoleInfo.do">
			     <table  class="tableopen"  width="100%">
			    	<tr>
			    	    <td class="text_rg title_font bzh">角色名称</td>
			    		<td class="text_lf">
			    		<input type="text" class="open_srk " name="roleName" id="roleName" value="${param.roleName}" />  
				         </td>
			    	    <td class="text_rg title_font bzh ">角色编码</td>
			    		<td class="text_lf">
			    			<input type="text" class="open_srk " name="roleCode" id="roleCode" value="${param.roleCode}" />
			    		</td>
			    	</tr>
		    	</table>
		 ...
		 <div class="row-fluid aufive mar_2"> 
		   <div class="aulist_3" ><b></b>角色列表</div>  
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	<tr class="tbth">
			    		<td class="bzh">选择</td>
			    		<td class="bzh">角色Id</td>
			    		<td class="bzh">角色编码</td>       		
			    		<td class="bzh">角色名称</td>
			    	</tr>
			    	<c:forEach var="list"  items="${list}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.roleId}','${list.roleName}')">选择</a>
						    </td>
				    		<td title="${list.roleId}">${list.roleId}</td>
				    		<td title="${list.roleCode}">${list.roleCode}</td>
				    		<td title="${list.roleName}">${list.roleName}</td>     		
				    	</tr>
				    </c:forEach> 
			    </table>    
		   		<tags:paginationOpen/>
