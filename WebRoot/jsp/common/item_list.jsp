...
<script type="text/javascript">
	/**产品编码、产品名称***/
	function btn_click(itemId,itemCode,itemName){
		parent.itemSet(itemId,itemCode,itemName);
		parent.closeDialog();
	}
</script>
<style>
	...
	<body style="border:0px;overflow:auto;height:500px;height:480px\9;">
		...
		<!--搜索部分begin-->	
		 <div class="row-fluid aufour mar_2"> 
		 	<form class="form-search" action="${ctx}/getItemList.do">
				<input type="hidden" name="url" value="${param.url}"/>
			     <table  class="tableopen"  width="100%">
			    	<tr>
			    		...
			    		<td class="text_lf">
			    			<input type="text" class="open_srk " name="itemName" id="itemName" value="${param.itemName}">
				    	</td>
			    	</tr>
		    	</table>
	    	...
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	...
			    	<c:forEach var="list"  items="${itemList}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.itemId}','${list.itemCode}','${list.itemName}')">选择</a>
						    </td>
				    		<td title="${list.itemCode}">${list.itemCode}</td>
				    		<td title="${list.itemName}">${list.itemName}</td>
				    		<td title="${list.unit}">${list.unit}</td>				    		
				    	</tr>
				    </c:forEach> 
			    </table>    
		   		<tags:paginationOpen/>
