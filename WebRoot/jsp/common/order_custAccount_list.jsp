...
<script type="text/javascript">
	function btn_click(accountNumber,accountName,custAccountId){
		parent.custAcountSet(accountNumber,accountName,custAccountId);
		parent.closeDialog();
	}
</script>
<style>
	...
	<body style="border:0px;overflow:auto;height:500px;height:480px\9;">
		...
		<!--搜索部分begin-->	
		 <div class="row-fluid aufour mar_2"> 
		   	<form class="form-search" action="${ctx}/getCuxAccount.do">
				...
				<input type="hidden" name="url" id="url" value="${url}"/>
			     <table  class="tableopen"  width="100%">
			    	<tr>
			    		...
			    		<td class="text_lf">
			    			<input type="text" class="open_srk " name="accountName" id="accountName" value="${param.accountName}" />
			    		</td>
			    	</tr>
			    		<tr>
			    	<td class="text_rg title_font bzh ">经销商子库</td>
					<td class="text_lf"><input type="text" class="open_srk " name="secInvName" id="secInvName" value="${param.secInvName}" /></td>
			    	</tr>
		    	</table>
	    	...
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	<tr class="tbth">
			    		...
			    		<td class="bzh">经销商子库</td>
			    		<td class="bzh">渠道</td>
			    	</tr>
			    	<c:forEach var="list"  items="${list}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.accountNumber}','${list.accountName}','${list.custAccountId}')">选择</a>
						    </td>
