...
<script type="text/javascript">
	...
	function btn_click(personId,userAccount,userName){
		parent.personCommitSetOrganization(personId,userAccount,userName);
		parent.closeCommitDialog();
	}
</script>
<style>
...
<body style="border:0px;overflow:auto;height:400px;">
 <form id='searchForm' class="form-search" action="${ctx}/getPersonCommitOrganization.do">
 		...
 		<input type="hidden" name="hr_organization_id" value="${hr_organization_id}"/> 
			    		...
			    		<td class="text_lf">
			    			...
			    			<option value="">请选择</option>
						       <c:forEach var="list"  items="${departmemtOrganizationList}" varStatus="vstatus">
						           		<option value="${list.hrOrganizationId}" <c:if test="${hr_organization_id eq list.hrOrganizationId}">selected='selected'</c:if>>${list.hrOrganizationName}</option>
						        </c:forEach>
					          </select>
			    		</td>
			    	</tr>
	    	...
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	...
			    	<c:forEach var="list"  items="${personList}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.personId}','${list.userAccount}','${list.userName}')">选择</a>
						    </td>
				    		<td>${list.orgName}&nbsp;</td>
