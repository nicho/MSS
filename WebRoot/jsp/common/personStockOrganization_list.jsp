...
<script type="text/javascript">
	...
	function btn_click(personId,userName,departmentCode,departmentName,positionId,positionName,orgId, userId,service_start_dateStr,province){
		parent.personSetOrganization(personId,userName,departmentCode,departmentName,positionId,positionName,orgId, userId,service_start_dateStr,province);
		parent.closeDialog();
	}
</script>
<style>
...
<body style="border:0px;overflow:auto;height:400px;">
 <form id='searchForm' class="form-search" action="${ctx}/getPersonOrganization.do">
 		...
 		<input type="hidden" name="provinceDisplay" value="${param.provinceDisplay}"/>
	    	...
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	...
			    	<c:forEach var="list"  items="${personList}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.personId}','${list.userName}','${list.departmentCode}','${list.departmentName}','${list.positionId}','${list.positionName}','${list.orgId}', '${list.userId}', '${list.service_start_dateStr}','${list.province}')">选择</a>
						    </td>
				    		<td>${list.orgName}&nbsp;</td>
