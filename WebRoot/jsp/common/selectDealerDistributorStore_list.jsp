...
<script type="text/javascript">
	/**经销商/分销商/门店编号、经销商/分销商/门店姓名、部门（省区）、岗位ID、岗位名称***/
	function btn_click(storeCode,storeName) {
		parent.personSetStore(storeCode,storeName);
		parent.closeDialog();
	}
</script>
<style>
...
<input type="hidden" name="url" id="url" value="${param.url}"/>
	<div class="add_top">&nbsp;&nbsp;&nbsp;●&nbsp;&nbsp;经销商/分销商/门店选择</div>
	<!--搜索部分begin-->
	<div class="row-fluid aufour mar_2">
		<form class="form-search" action="${ctx}/getSelectDealerDistributorStore.do">
			<input type="hidden" name="url" value="${param.url}" />
			<table class="tableopen" width="100%">
				<tr>
					<td class="text_rg title_font bzh">经销商/分销商/门店编码</td>
					<td class="text_lf"><input type="text" class="open_srk " name="storeCode" id="storeCode" value="${param.storeCode}"></td>
					<td class="text_rg title_font bzh">经销商/分销商/门店名称</td>
					<td class="text_lf"><input type="text" class="open_srk " name="storeName" id="storeName" value="${param.storeName}"></td>
				</tr>
			</table>
	...
	<div class="row-fluid aufive mar_2">
		...
		<div class="table_auto scroll_auto" style="overflow-x: auto; overflow-y: hidden;">
			<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
				<tr class="tbth">
					<td class="bzh">选择</td>
					<td class="bzh">经销商/分销商/门店编码</td>
					<td class="bzh">经销商/分销商/门店名称</td>

				</tr>
				<c:forEach var="list" items="${storeDtoList}" varStatus="vstatus">
					<tr>
						<td><a onclick="btn_click('${list.storeCode}','${list.storeName}')">选择</a></td>
						<td title="${list.storeCode}">${list.storeCode}</td>
						<td title="${list.storeName}">${list.storeName}</td>
