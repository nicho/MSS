...
<html>
<head> 
<title><c:if test="${isChannel!='N'}">经销商</c:if>子库选择</title>
...
<script type="text/javascript">
	function btn_click(secInvName,description){
		parent.vendorLovSet(secInvName,description);
		parent.closeDialog();
	}
</script>
<style>
...
.open_xlk{width:200px;height:25px;overflow:hidden;border:1px solid #ccc;margin-left:8px\9;font-size:12px!important;}
	...
	<body style="border:0px;overflow:auto;height:500px;height:480px\9;">
		<div class="add_top" style>&nbsp;&nbsp;&nbsp;●&nbsp;&nbsp;<c:if test="${isChannel!='N'}">经销商</c:if>子库选择</div>
		<!--搜索部分begin-->	
		 <div class="row-fluid aufour mar_2"> 
		   	<form class="form-search" action="${ctx}/getVendorLov.do">
				<input type="hidden" name="orgId" id="orgId" value="${orgId}"/>
				<input type="hidden" name="url" id="url" value="${url}"/>
				<input type="hidden" name="isChannel" id="isChannel" value="${isChannel}"/>
				<input type="hidden" name="isUser" id="isUser" value="${isUser}"/>
				<input type="hidden" name="igNore" id="igNore" value="${igNore}"/>
			     <table  class="tableopen"  width="100%">
			    	<tr>
			    	    <td class="text_rg title_font bzh"><c:if test="${isChannel!='N'}">经销商</c:if>子库编码</td>
			    		<td class="text_lf">
			    		<input type="text" class="open_srk " name="secInvName" id="secInvName" value="${param.secInvName}" />  
				         </td>
			    	    <td class="text_rg title_font bzh "><c:if test="${isChannel!='N'}">经销商</c:if>子库名称</td>
			    		<td class="text_lf">
		 ...
		 <div class="row-fluid aufive mar_2"> 
		   <div class="aulist_3" ><b></b><c:if test="${isChannel!='N'}">经销商</c:if>子库列表</div>  
	    	<div class="table_auto scroll_auto" style="overflow-x:auto;overflow-y:hidden;">
		 		<table id="contentTable" width="90%" class="table table-striped table-bordered table-condensed table-hover ">
			    	<tr class="tbth">
			    		<td class="bzh">选择</td>
			    		<td class="bzh"><c:if test="${isChannel!='N'}">经销商</c:if>子库编码</td>     		
			    		<td class="bzh"><c:if test="${isChannel!='N'}">经销商</c:if>子库名称</td>
			    		<td class="bzh">省</td>
			    		<td class="bzh">市</td>
			    		<c:if test="${igNore!='Y'}">
			    		<td class="bzh">渠道</td>
			    	...
			    	<c:forEach var="list"  items="${list}" varStatus="vstatus">
				    	<tr>
				    		<td>
						    	<a onclick="btn_click('${list.secInvName}','${list.description}')">选择</a>
						    </td>
				    		...
				    		<td title="${list.provinceName}">${list.districtName}</td> 
				    		<c:if test="${igNore!='Y'}">
				    		<td title="${list.channelType}">
